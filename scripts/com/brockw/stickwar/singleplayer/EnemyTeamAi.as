package com.brockw.stickwar.singleplayer
{
   ...
   public class EnemyTeamAi
   {
      ...
      protected function rebalanceMiners(game:StickWar) : void
      {
         ...
         if(_loc3_.length + _loc2_.length > 0)
         {
            ...
            else if(_loc3_.length > _loc6_)
            {
               _loc5_ = _loc3_[0];
               MinerAi(_loc5_.ai).targetOre.releaseMiningSpot(_loc5_);
               this.iterateOverFreeMines(_loc5_,game);
            }
         }
      }
      ...
      protected function iterateOverFreeMines(miner:Miner, game:StickWar) : void
      {
         ...
         {
            for(_loc4_ = 0; _loc4_ < game.map.gold.length; _loc4_++)
            {
               _loc5_ = game.map.gold[_loc4_];
               if(this.getFreeMine(miner,game,_loc5_))
               {
                  _loc3_ = true;
                  break;
               }
         ...
         {
            for(_loc4_ = game.map.gold.length - 1; _loc4_ >= 0; _loc4_--)
            {
               _loc5_ = game.map.gold[_loc4_];
               if(this.getFreeMine(miner,game,_loc5_))
               {
