package com.brockw.stickwar.engine.Ai
{
   ...
   public class MinerAi extends UnitAi
   {
      ...
      override public function update(game:StickWar) : void
      {
         ...
         else if(this.targetOre != null && !unit.isGarrisoned)
         {
            ...
            if(Miner(unit).isBagFull())
            {
               ...
               _loc3_ = game.map.height / 2 - unit.py;
      ...
      protected function iterateOverFreeMines(miner:Miner, game:StickWar) : void
      {
         ...
         {
            for(_loc3_ = 0; _loc3_ < game.map.gold.length / 2; _loc3_++)
            {
               _loc4_ = game.map.gold[_loc3_];
               if(this.getFreeMine(miner,game,_loc4_))
               {
                  break;
               }
            }
         ...
         {
            for(_loc3_ = game.map.gold.length - 1; _loc3_ >= game.map.gold.length / 2; _loc3_--)
            {
               _loc4_ = game.map.gold[_loc3_];
               if(this.getFreeMine(miner,game,_loc4_))
               {
