package com.brockw.stickwar.engine.Ai.command
{
   ...
   public class UnitCommand
   {
      ...
      public function prepareNetworkedMove(gameScreen:GameScreen) : *
      {
         ...
         var _loc4_:Number = 0;
         for(_loc5_ in gameScreen.team.units)
         {
            if(this._isSingleSpell)
            {
               _loc11_ = UnitCommand(gameScreen.game.commandFactory.createCommand(gameScreen.game,this.type,0,0,0,0,0,0,0));
               if((this.intendedEntityType == -1 || this.intendedEntityType == gameScreen.team.units[_loc5_].type) && Unit(gameScreen.team.units[_loc5_]).selected && (!_loc11_.hasCoolDown || _loc11_.hasCoolDown && _loc11_.coolDownTime(gameScreen.team.units[_loc5_]) == 0) && !Unit(gameScreen.team.units[_loc5_]).isBusy())
               {
                  _loc12_ = Unit(gameScreen.team.units[_loc5_]).px - this.realX * Unit(gameScreen.team.units[_loc5_]).px - this.realX + Unit(gameScreen.team.units[_loc5_]).py - this.realY * Unit(gameScreen.team.units[_loc5_]).py - this.realY;
                  if(_loc3_ == null)
                  {
                     _loc3_ = gameScreen.team.units[_loc5_];
                     _loc4_ = _loc12_;
                  }
            ...
            else if(Unit(gameScreen.team.units[_loc5_]).selected)
            {
               if(this.intendedEntityType == -1 || this.intendedEntityType == gameScreen.team.units[_loc5_].type)
               {
                  _loc2_.units.push(gameScreen.team.units[_loc5_].id);
               }
            }
         }
         if(_loc3_ != null)
         ...
         for(_loc5_ in gameScreen.team.walls)
         {
            if(this._isSingleSpell)
            {
               if(Unit(gameScreen.team.walls[_loc5_]).selected)
               {
                  if(this.intendedEntityType == -1 || this.intendedEntityType == gameScreen.team.walls[_loc5_].type)
                  {
                     _loc2_.units.push(gameScreen.team.walls[_loc5_].id);
                  }
               }
            }
         }
         _loc2_.arg0 = gameScreen.game.battlefield.mouseX;
         _loc2_.arg1 = Math.max(0,Math.min(gameScreen.game.map.height,gameScreen.game.battlefield.mouseY));
         _loc6_ = gameScreen.userInterface.hud.hud.map.mouseX / gameScreen.userInterface.hud.hud.map.width;
         _loc7_ = gameScreen.userInterface.hud.hud.map.mouseY / gameScreen.userInterface.hud.hud.map.height;
         _loc8_ = gameScreen.userInterface.hud.hud.map.globalToLocal(new Point(gameScreen.userInterface.mouseState.mouseDownX,gameScreen.userInterface.mouseState.mouseDownY));
         _loc9_ = _loc8_.x / gameScreen.userInterface.hud.hud.map.width;
         _loc10_ = _loc8_.y / gameScreen.userInterface.hud.hud.map.height;
         if(_loc6_ >= 0 && _loc6_ <= 1 && _loc7_ >= 0 && _loc7_ <= 1 && _loc9_ >= 0 && _loc9_ <= 1 && _loc10_ >= 0 && _loc10_ <= 1)
         {
            _loc2_.arg0 = _loc6_ * gameScreen.game.map.width;
            _loc2_.arg1 = _loc7_ * gameScreen.game.map.height;
         }
