package com.brockw.stickwar.engine.Team
{
   ...
   public class Team
   {
      ...
      public function dequeueUnit(type:int, backwards:Boolean) : Unit
      {
         var _loc5_:int = 0;
         var _loc6_:Unit = null;
         var _loc3_:int = this.unitInfo[type][2];
      ...
      public function checkUnitCreateMouseOver(gameScreen:GameScreen) : void
      {
         ...
         var _loc2_:int = gameScreen.stage.mouseX;
         var _loc3_:int = gameScreen.stage.mouseY;
         ...
         for(_loc6_ in this.buttonInfoMap)
         {
            ...
            else
            {
               ...
               {
                  _loc9_.visible = true;
                  if(_loc9_.hitTestPoint(_loc2_,_loc3_,false) && gameScreen.userInterface.mouseState.clicked)
                  {
                     _loc15_ = new UnitCreateMove();
                     _loc15_.unitType = -int(_loc6_);
                     gameScreen.doMove(_loc15_,this.id);
                     _loc4_ = true;
                  }
               }
               else
      ...
      protected function singlePlayerDebugInputSwitch(userInterface:UserInterface, unitType:int, key:int) : void
      {
         ...
         if(userInterface.keyBoardState.isPressed(key))
         {
            ...
            {
               _loc4_.unitType = unitType;
               userInterface.gameScreen.doMove(_loc4_,userInterface.gameScreen.team.enemyTeam.id);
            }
            ...
            {
               _loc4_ = new UnitCreateMove();
               _loc4_.unitType = int(unitType);
               userInterface.gameScreen.doMove(_loc4_,this.id);
               this.game.soundManager.playSoundFullVolume("UnitMake");
            }
         }
      }
      ...
      public function update(game:StickWar) : void
      {
         ...
         if(game.map.hills.length != 0)
         {
            _loc8_ = game.map.hills[0].getControllingTeam(game);
            if(_loc8_ == this)
            {
         ...
         _loc3_ = game.gameScreen is SingleplayerGameScreen;
         for each(_loc4_ in this._unitProductionQueue)
         {
            if(_loc4_.length != 0)
            {
               ...
               {
                  this.spawn(Unit(_loc4_[0][0]),game);
         ...
         if(this._units.length != 0)
         {
            ...
            for(_loc6_ in this._units)
            {
               ...
               {
                  if(!this._units[_loc6_].isSlow() || game.frame % 2 == 0)
                  {
                     this._units[_loc6_].update(game);
                  }
               }
            }
         }
         ...
         {
            this._deadUnits[_loc6_].update(game);
         }
         ...
         _loc7_.owner = this.id;
         _loc7_.arg0 = this.homeX - this.direction * game.map.screenWidth / 3;
         _loc7_.arg1 = game.map.height / 2;
         _loc7_.execute(game);
      }
