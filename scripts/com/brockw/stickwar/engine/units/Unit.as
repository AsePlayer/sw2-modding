      ...
      private function checkCollision(u:Unit) : void
      {
         ...
         if(u is Unit)
         {
            ...
            if(!u.mayWalkThrough && u.stunTimeLeft == 0 && u.isCollision(this,this.dx,this.dy))
            {
               ...
               _loc4_ = u.ai.currentCommand.type;
               _loc5_ = _loc3_ == UnitCommand.STAND;
               _loc6_ = _loc4_ == UnitCommand.STAND;
               _loc7_ = (u.hitBoxWidth * u.perspectiveScale + this.hitBoxWidth * this.perspectiveScale) / 2;
               _loc8_ = Math.abs(px - u.px) / _loc7_;
               if(_loc8_ <= 1)
               {
                  if(px == u.px)
                  {
                  ...
                  if(px != u.px)
                  {
                     if(Util.sgn(this._dx) != -Util.sgn(u.dx) || this.team != u.team)
                     {
                        if(Math.abs(this.dx) < 1 && Math.abs(u.dx) < 1 || u.isMiner() && this.isMiner())
                        {
                           ...
                           if(this.mass < u.mass || _loc5_)
                           {
                              this._dx += (1 - _loc8_) * Util.sgn(px - u.px) * 0.51;
                           }
                           if(this.mass >= u.mass || _loc6_)
                           {
                              u._dx += (1 - _loc8_) * Util.sgn(u.px - px) * 0.51;
                           }
                        }
                     }
                  }
               ...
               _loc7_ = (u.hitBoxWidth * u.perspectiveScale + this.hitBoxWidth * this.perspectiveScale) / 2;
               _loc8_ = Math.abs(py - u.py) / _loc7_;
               if(_loc8_ <= 1)
               {
                  if(py == u.py)
                  {
                  ...
                  if(py != u.py)
                  {
                     if(Util.sgn(this._dy) != -Util.sgn(u.dy) && this.team == u.team)
                     {
                        ...
                        if(Math.abs(this.dy) < 1 && Math.abs(u.dy) < 1 || u.isMiner() && this.isMiner())
                        {
                           if(this.mass < u.mass || _loc5_)
                           {
                              this._dy += (1 - _loc8_) * Util.sgn(py - u.py) * 0.51;
                           }
                           if(this.mass >= u.mass || _loc6_)
                           {
                              u._dy += (1 - _loc8_) * Util.sgn(u.py - py) * 0.51;
                           }
                        }
                     }
                  }
